...
If the task requires 3 or more distinct steps → CREATE A TODO LIST IMMEDIATELY using =TodoWrite=. This is not optional.
...
- *NEVER use executor to fix code issues* — executor runs Haiku which can follow explicit orders but cannot diagnose or fix problems.  Use =executor-writer= instead for any fix that requires understanding what went wrong
...
- *Fixing errors found by tests or compilation* — syntax errors, type errors, missing imports, broken references.  Include the error output and relevant file contents in the delegation prompt so executor-writer can diagnose and fix without re-gathering
...
- *Tests or compilation failed with errors* → =executor-writer= (NEVER fix inline, NEVER use executor)
- *Docs need updating* → =executor-writer=
