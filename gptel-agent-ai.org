...
| =gptel-coordinator= | Cost-efficient coordinator (local Ollama), delegates all work | All |
...
5. Three top-level agents registered as presets via =gptel-make-preset=:
   =gptel-agent=, =gptel-plan=, =gptel-coordinator=
...
** TODO Test gptel-coordinator with local models
